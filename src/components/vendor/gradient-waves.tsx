// Gradient Waves — React Bits. Codigo de terceiro, mantido como veio.
//
// Um plano de ondas desenhado inteiro num fragment shader: nao existe geometria,
// so um triangulo cobrindo a tela e uma funcao que, pra cada pixel, decide onde a
// crista da onda estaria. O ponteiro entra como uniform e inclina o campo.
//
// Alteracoes locais em relacao ao original:
//   1. Adicionado o "use client".
...
//      `pointer-events: none` — nesse caso ele nao recebe evento nenhum e a
//      interacao fica inerte. Com "window" ele escuta na janela e converte a
//      posicao para as coordenadas do canvas. O padrao continua sendo "element",
//      o comportamento original. Mesma alteracao ja feita em flow-ribbons.
//   3. Guarda contra `rect` de tamanho zero, que apareceu junto com a mudanca
//      acima: escutando na janela o handler pode disparar antes do primeiro
//      layout, e dividir por zero manda NaN pro shader.
//   4. Nada mais. Se atualizar, recopiar do React Bits.
...
export type PointerSource = 'element' | 'window';
...
const GradientWaves: React.FC<GradientWavesProps> = ({
  ...
  useEffect(() => {
    ...
    // Alteracao local: ver o cabecalho.
    const pointerTarget: HTMLElement | Window = pointerSource === 'window' ? window : canvas;
